...
def clusterevaluate(labelstrue,predicted,clustermetric):
    ...
    if clustermetric=="rand": #adjusted rand index
         pass
    elif clustermetric=="fmeasure": #fmeasure
         return F(predicted,labelstrue)[0]
    elif clustermetric=="precision": #precision
         return F(predicted,labelstrue)[1]
    elif clustermetric=="recall": #recall
         return F(predicted,labelstrue)[2]
...
def writegraphmyline(outpath,phy,weighted): 
    ...
    if weighted:
       for edge in linegraph.edges_iter():
           ...
           if len(val1)!=1:
              print "ERROR: length of list is not 1 but {0}".format(len(val1))
              exit(1)
           val1=val1[0]
           val2=list(set2.difference(set1))
           if len(val2)!=1:
              print "ERROR: length of list is not 1 but {0}".format(len(val2))
              exit(1)
           val2=val2[0]
    ...
    for line in file:
        ...
        if not edgeflag:
           ...
           if not index2node.has_key(index):
              ...
              print "error index {0} already exists!!".format(index)
              exit(1)
        else:
            parts=line.split()
            if compressed:
...
def estimateoverlapmetric(communities,metrictype):
    if metrictype=="overlap":
       ...
       total=0.0
       for index1 in range(0,len(commnunities)):
           com1=communities[index1]
           for index2 in range(index1+1,len(commnunities)): 
               com2=communities[index2]
               maxcom=float(max([len(com1),len(com2)]))
               total += (len(com1.intersection(com2))/maxcom)
       ...
       for index1 in range(0,len(commnunities)):
           ...
           for index2 in range(index1+1,len(commnunities)): 
               com2=communities[index2]
               maxcom=float(max([len(com1),len(com2)]))
               intnum=len(com1.intersection(com2))/maxcom
               total += min(intnum,1.0-intnum)
       return float(total)/norm     
    else:
       print "{0} metric is UNKNOWN!!".format(metrictype)
       exit(1)
...
def  annofilereader(filename,minsizelimit,annoint=True):
     ...
     for line in file:
         ...
         if annoint:    
            ...
            allannotations|=set(intparts)    
            if pro2anno.has_key(parts[0]):
               pro2anno[parts[0]]|=set(intparts)     
            else:
               pro2anno[parts[0]]=set()
               pro2anno[parts[0]]|=set(intparts)
         else:
            allannotations|=set(parts[1:])    
            if pro2anno.has_key(parts[0]):
               pro2anno[parts[0]]|=set(parts[1:])     
            else:
               pro2anno[parts[0]]=set()
               pro2anno[parts[0]]|=set(parts[1:])
     file.close()
...
def nodeswithindistance(ppi,pro,radius):
    if radius<1:
        return set()
    elif radius==1:
        return set(G.neighbors(protein))
    else:
        retlist=set()
        propath=nx.single_source_shortest_path_length(G,protein)
        for pro in propath.keys():
            if propath[pro]<=radius:
                retlist.add(pro)
        return retlist
