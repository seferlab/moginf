...
graphfilename="loc-gowalla_edges.txt"
checkinfilename="loc-gowalla_totalCheckins.txt"
...
file=open(checkinfilename,"r")
for line in file:
    line=line.rstrip()
    userid,timeinfo,latitude,longtitude,locid=line.split("\t")
    userid=int(userid)
    date,time=timeinfo.split("T")
    year,month,day=date.split("-")
    hour,minute,second=time.replace("Z","").split(":")
    year=int(year)
    month=int(month)
    day=int(day)
    hour=int(hour)
