...
def criticalpointroundtrace(allnodestate,traceroundingmethod,traceroundingparam,spreadmodel):
    ...
    if spreadmodel=="si": #avginfected will be important for determining whether round sth eventually to infected state
       infecttimes={} 
       if traceroundingmethod=="categorical": #categorical dist rounding
       ...
       print "rounding mechanism for seir has not yet implemented!!"
       exit(1)
...
#truegraphfilepath will be used as param but won't be useful
#netinf assumes graph is directed
def si_static_netinf(allnodestates,outrunfolder,s2idist,traceroundingmethod,traceroundingparam,codedirname,iterationcount,tracenoisyornot):
    print "inside netinf!!"
...
def si_static_connie(allnodestates,outrunfolder,s2idist,traceroundingmethod,traceroundingparam,codedirname,lambda1,matlabpath,tracenoisyornot):
    spreaddist=s2idist[0]
    spreadparam=s2idist[1:]
...
def si_static_netrate(allnodestates,outrunfolder,s2idist,traceroundingmethod,traceroundingparam,codedirname,windowsize,matlabpath,allnodenum,tracenoisyornot):
    spreaddist=s2idist[0]
    spreadparam=s2idist[1:]
    if spreaddist=="expo":
       disttype='exp'
...
def si_static_multitree(allnodestates,outrunfolder,s2idist,traceroundingmethod,traceroundingparam,codedirname,iterationnum,usedcascadenum,tracenoisyornot):
    spreaddist=s2idist[0]
    spreadparam=s2idist[1:]
    if spreaddist=="expo":
       disttype=0
       alpha=1.0/spreadparam[0]
       delta=spreadparam[0] #not important, just random
    elif spreaddist=="powerlaw":
       disttype=1
       alpha=-1.0-(1.0*spreadparam[0])
       delta=1.0
       ...
       disttype=2
       alpha=1.0/(spreadparam[0]**2)
       delta=1.0/(spreadparam[0]**2) #not important, just random
    else:
...
def otheralgoinference(allnodestates,allnodestatesinfo,algo,algoparams,specificrunfolder,traceroundingmethod,traceroundingparam,tracenoisyornot):
    ...
    if algo=="netinf":
       ...
       codedirname="./algos/netinf-unix"
       retvalues=si_static_netinf(allnodestates,specificrunfolder,s2idist,traceroundingmethod,traceroundingparam,codedirname,iterationcount,tracenoisyornot)   
    elif algo=="netrate":
       ...
       print matlabpath
       print allnodenum
       retvalues=si_static_netrate(allnodestates,specificrunfolder,s2idist,traceroundingmethod,traceroundingparam,codedirname,windowsize,matlabpath,allnodenum,tracenoisyornot)
       ...
       retvalues=si_static_connie(allnodestates,specificrunfolder,s2idist,traceroundingmethod,traceroundingparam,codedirname,lambda1,matlabpath,tracenoisyornot)
    elif algo=="multitree":
       codedirname="./algos/multitree"
       iterationnum=algoparams[0]
       usedcascadenum=algoparams[1]
       retvalues=si_static_multitree(allnodestates,specificrunfolder,s2idist,traceroundingmethod,traceroundingparam,codedirname,iterationnum,usedcascadenum,tracenoisyornot)
    return retvalues
...
def recoverlseprobassigner(allnodestate,sortedtimes,recoverprobdistcoef):
    ...
    for node in allnodestate.keys():
        ...
        optx=np.linalg.lstsq(A,b)[0] #this the solution
        modifiedoptx=[]
        for elem in optx:
            if elem<=0:
               modifiedoptx.append(0.0)
            else:
               modifiedoptx.append(elem) 
        assert len(modifiedoptx)==timeduration-1
        retprobs[node]={}
        for timeindex in range(0,timeduration-1):
            mytime=sortedtimes[timeindex]
            retprobs[node][mytime]=modifiedoptx[timeindex]
        weightedsum=0.0
...
def constgenerate(allnodestates,allnodestatesinfo,graphtype,logbase,graphevolution,spreadmodel,rightmethod,rightmethodparam,timesteplist,infectedprobabilityassignermethod,infertype,partialdegree,sumormul,temporalsparsetype,mapintervals): 
    con="?"
    ...
    for traceindex in range(0,len(allnodestates)):
        ...
        if infertype=="edge":
           if spreadmodel in ["seir"]:
              print "writing!!" 
              print s2eparam
              probdistcoef=tracegeneratorpart.partitiondist(s2eparam[0],s2eparam[1:],"reverse cumulative",spreadparam)
           elif spreadmodel in ["si","sir","sis"]:    
              probdistcoef=tracegeneratorpart.partitiondist(s2iparam[0],s2iparam[1:],"reverse cumulative",spreadparam)
           else:
              print "probdistcoef assignment has not been defined for model {0}".format(spreadmodel)
              exit(1)
           if spreadmodel in ["sir","seir"]:
              recoverprobdistcoef=tracegeneratorpart.partitiondist(i2rparam[0],i2rparam[1:],"normal")
        elif infertype=="spreadprobunknown": #?
           probdistcoef=tracegeneratorpart.partitiondist(s2iparam[0],s2iparam[1:],"reverse cumulative",spreadparam)
        elif infertype=="partialdifprob": #?
           probdistcoef=tracegeneratorpart.partitiondist(s2iparam[0],s2iparam[1:],"reverse cumulative",spreadparam)
        elif infertype=="difprobunknown": #?m
           probdistcoef=tracegeneratorpart.partitiondist(s2iparam[0],s2iparam[1:],"reverse cumulative",spreadparam)   
        ...
        if partialdegree!=1.0:
           ...
           if infectedprobabilityassignermethod=="flowdifference":
              ...
              assert spreadmodel in ["sir","seir"]
            ...
            for timeindex in range(1,len(sortedtimes)):
                ...
                if infertype=="edge":
                   if spreadmodel in ["sir","seir","si","sis"]:
                      ...
                      edge_othermodels_constgenerate()
                elif infertype=="spreadprobunknown":
                   continue   
                elif infertype=="difprobunknown":
                   continue 
                elif infertype=="partialdifunknown":     
                   continue
                substr+=localsubstr
...
def lseinference(allnodestates,allnodestatesinfo,specificrunfolder,graphevolution,spreadmodel,Gname,algo,graphtype,logbase,lambda1,lambda2,fusedlambda,temprightmethod,timesteplist,infectedprobabilityassignermethod,infertype,partialdegree,mapintervals):
    ...
    #if infertype in ["difprobpartial","spreadprobunknown"]:
    ...
    #                      print "ERROR:Trace info is not perfect but noisy. This is not accepted in difprobpartial case!!!" 
    #                      exit(1)
    ...
    con="?"
    consstr,consnum,edge2name=constgenerate(allnodestates,allnodestatesinfo,graphtype,logbase,graphevolution,spreadmodel,rightmethod,rightmethodparam,timesteplist,infectedprobabilityassignermethod,infertype,partialdegree,sumormul,temporalsparsetype,mapintervals) #timessteplist will be used for dynamic graph inference
