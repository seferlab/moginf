...
import operator
import heapq
import offlineinferenceoptimizationalgos as edgeinferenceoptalgos
import community
import gzip
import cPickle
import threading
...
#time here are probability vector index
def estimatevectorscore(G,ourG,graphtype,graphevolution):
    assert graphevolution not in ["dynamic"]
    if graphtype=="directed":
       assert nx.DiGraph==type(G) and nx.DiGraph==type(ourG) 
    elif graphtype=="undirected":
       assert nx.Graph==type(G) and nx.Graph==type(ourG)     
    ...
    for time in alltimes:
        if ourG.has_key(time):
           alledges |= ourG[time].edges()    
    for node1,node2 in alledges: 
        ...
        for time in alltimes:
            if not G.has_key(time):
               ...
               if not G[time].has_key((node1,node2)):
                  vec1.append(0.0)
               else:    
            ...
            if not ourG.has_key(time):
               ...
               if not ourG[time].has_key((node1,node2)):
                  vec2.append(0.0)
               else:    
                  vec2.append(ourG[time][node1,node2])
            #assert check for probability vectorsness     
...
#scale dependent: mabse,mse,rmse and scale independent: nmabse,nmse,nrmse
def estimateerrorscore(G,ourG,graphtype):
    assert graphevolution not in ["dynamic"] #but dynamic can easily be done similar to edge one
    if graphtype=="directed":
       assert nx.DiGraph==type(G) and nx.DiGraph==type(ourG) 
    elif graphtype=="undirected":
       assert nx.Graph==type(G) and nx.Graph==type(ourG)
    ...
    maxval=-1000000000.0
    minval=100000000000.0
    for node1,node2 in G.edges():
        if G[node1][node2]["weight"]>maxval:
           maxval=G[node1][node2]["weight"]
        if G[node1][node2]["weight"]<minval:
           minval=G[node1][node2]["weight"]
    for node1,node2 in ourG.edges():
        if ourG[node1][node2]["weight"]>maxval:
           maxval=ourG[node1][node2]["weight"]
        if ourG[node1][node2]["weight"]<minval:
           minval=ourG[node1][node2]["weight"]
    nmse=mse/(float(maxval-minval)**2)
    nrmse=rmse/float(maxval-minval)
    nmabse=mabse/float(maxval-minval)
...
def graphinference(alltraces,alltracesinfo,uniquecount,timestepslist):   
    ...
    if infertype in ["edge"]:
       if spreadmodel=="si":
          ...
          spreadparam1,s2iparam,spreadparam2,i2sparam=specificparam 
       elif spreadmodel=="model2sum":
          spreadparam1,s2iparam,spreadparam2,i2rparam=specificparam 
       elif spreadmodel=="model3sum":
          print "no infer for this method {0}".formast(spreadmodel)
          exit(1)
       elif spreadmodel=="model4sum":
          print "no infer for this method {0}".formast(spreadmodel)
          exit(1)
       elif spreadmodel=="model5sum":
          spreadparam,s2iparam,i2sparam,modelpa=specificparam  
       elif spreadmodel=="singlemulti-si":
          print "NOT IMPLEMENTED YET method {0}".formast(spreadmodel)
          exit(1)
       elif spreadmodel=="multimulti-si":
          spreadparam,s2iparam,simultracenum,hardness=specificparam  
       elif spreadmodel=="multimulti-si-corr":
          spreadparam,s2iparam,simultracenum,hardness=specificparam  
       else:
          print "spreadmodel {0} is unknown".format(spreadmodel)
          exit(1)
    elif infertype in ["difprobpartial","macropartial"]:
        if spreadmodel=="si":
           spreadparam,s2iparam=specificparam
        elif spreadmodel=="sir":
           spreadparam,s2iparam,i2rparam=specificparam
        ...
        if spreadmodel=="si":
           ...
           print "spreadmodel {0} is unknown".format(spreadmodel)
           exit(1)
    elif infertype in ["spreadprobunknown"]:
        if spreadmodel=="si":
           spreadparam,s2iparam=specificparam
        elif spreadmodel=="sir":
           spreadparam,s2iparam,i2rparam=specificparam
       ...
       print "infertype {0} is unknown".format(infertype)
       exit(1)
    ...
    for tracefraction in tracefractions:
        ...
        for index in range(0,algoruncount):
            ...
            for sampleinterval in sampleintervals:
                #avgscore[sampleinterval]={}
                #if infertype in ["edge"]:
                #   if spreadmodel not in ["multimulti-si-corr"]:
                #      tempscorenames=edgepredictionscorenames
                #   else:   
                #      tempscorenames=errorscorenames
                #elif infertype in ["spreadprobunknown","difprobpartial","macropartial"]:
                #   tempscorenames=errorscorenames
                #elif infertype in ["difprobunknown"]:
                ...
                #   tempscorenames=errorscorenames
                #else:
                #   print "such case is unknown for scores inside inference {0}".format(infertype)
                #   exit(1)
                #for scorename in tempscorenames:
                #    avgscore[sampleinterval][scorename]="-1"
			   ...
			   if time%sampleinterval==0:
                ...
                if infertype in ["edge"] and algo in fixededgestaticcovermyalgos:
                   ...
                   print "cover case is noT IMPLEMENTED PROPERLY!!"
                   exit(1)
                   ...
                   print "cover case is noT IMPLEMENTED PROPERLY!!"
                   exit(1)
                elif infertype in ["macropartial"] and algo in fixedmacropartialcovermyalgos:
                   print "cover case is noT IMPLEMENTED PROPERLY!!"
                   exit(1)
                elif infertype in ["lag"] and algo in fixedlagcovermyalgos:
                   print "cover case is noT IMPLEMENTED PROPERLY!!"
                   exit(1)   
                   ...
                   print "cs case is noT IMPLEMENTED PROPERLY!!"
                   exit(1) 
                   ...
                   print "cs case is noT IMPLEMENTED PROPERLY!!"
                   exit(1)
                elif infertype in ["macropartial"] and algo in fixedmacropartialcsalgos:
                   print "cs case is noT IMPLEMENTED PROPERLY!!"
                   exit(1)
                elif infertype in ["lag"] and algo in fixedlagcsalgos:
                   print "cs case is noT IMPLEMENTED PROPERLY!!"
                   exit(1)
                elif infertype in ["edge"] and spreadmodel not in ["singlemulti-si","multimulti-si","multimulti-si-corr"] and ((algo in fixededgestaticoptmyalgos) or (algo in fixededgedynamicoptmyalgos)):
                   if spreadmodel in ["si","sir","sis","seir"]:
                      ...
                      retvalues=edgeinferenceoptalgos.mainoptinference(sampleallnodestates,allnodestatesinfo,specificrunfolder,graphevolution,spreadmodel,Gname,algo,graphtype,logbase,lambda1,lambda2,fusedlambda,rightmethod,timesteplist,infectedprobabilityassignermethod,infertype,partialdegree,mapintervals,probtype)
                   elif spreadmodel in ["model1sum","model2sum","model5sum"]:
                      pass
                      #need otheralgoinference
                   elif spreadmodel in ["model3sum","model4sum"]:
                       print "model {0} not impl".format(spreadmodel)
                       exit(1)
                   else:
                      print "spreadmodel {0} is unknown for type {1}".format(spreadmodel,infertype)
                      exit(1)
                elif infertype in ["edge"] and spreadmodel in ["singlemulti-si"] and algo in fixedsinglemultisiedgeoptmyalgos:
                   #dependent inference neeeed
                   pass 
                   ...
                   pass
                elif infertype in ["macropartial"] and (algo in fixedmacropartialoptmyalgos):
                   #need macro inference
                   pass 
                elif infertype in ["lag"] and (algo in fixedlagoptmyalgos):
                   pass
                   ...
                   if spreadmodel in ["si"]:
                      ...
                      retvalues=edgeinferenceoptalgos.otheralgoinference(sampleallnodestates,allnodestatesinfo,algo,newalgoparams,specificrunfolder,traceroundingmethod,traceroundingparam,tracenoisyornot,probtype)
                   else:
                      print "can not run other algos for this case {0} {1}".format(infertype,spreadmodel)
                      exit(1)
                   ...
                   matlabpath="/usr/local/bin/matlab"
                   #matlabpath="/opt/stow/matlab-r2012a/bin/matlab"
                   print "running connie for spreadprobunknown"
                   #traceroundingmethod=algoparams[0][0][0]
                   #traceroundingparam=algoparams[0][0][1]
                   print algoparams[0]
                   print algoparams[0][1]
                   ...
                   matlabpath="/usr/local/bin/matlab"
                   #matlabpath="/opt/stow/matlab-r2012a/bin/matlab"
                   print "running netrate for difprobpartial"
                   #traceroundingmethod=algoparams[0][0][0]
                   #traceroundingparam=algoparams[0][0][1]
                   print algoparams[0]
                   print algoparams[0][1]
                   ...
                   newalgoparams.append(G.number_of_nodes())
                   retvalues=edgeinferenceoptalgos.otheralgoinference(sampleallnodestates,allnodestatesinfo,algo,newalgoparams,specificrunfolder,traceroundingmethod,traceroundingparam,tracenoisyornot,probtype)
                elif infertype in ["macropartial"] and (algo in fixedmacropartialotheralgos):
                   print "spreadmodel {0} is unknown for type {1}".format(spreadmodel,infertype)
                   exit(1)
                elif infertype in ["lag"] and (algo in fixedlagotheralgos):
                   print "spreadmodel {0} is unknown for type {1}".format(spreadmodel,infertype)
                   exit(1)
                else:
                   print "No other comparison algorithm to run for model {0} {1}".format(spreadmodel,graphevolution)
                   print "THIS CASE DOES NOT FIT ANY OTHER CASE!!"
                   exit(1)
                ...
                for roundingmethod in usefulroundingmethods:
                    ...
                    if infertype in ["edge"]: #rounding is here
                    ...
                    elif infertype in ["spreadprobunknown","difprobpartial","macropartial","lag"]:
                       if graphtype=="undirected": #what if retvalues are directed, this can only happen on other algos
                          ourG=nx.Graph()
                          for node1,node2 in usedretvalues.keys():
                              ourG.add_edge(node1,node2,weight=usedretvalues(node1,node2))
                       ...
                       #??make sure G and ourG are probability vectors of the form estimatevectorscore???      
                       scores=estimatevectorscore(G,ourG,graphtype,graphevolution) 
...
#can we delete this and use the one in tracegeneratorpart instead???? 
...
fixedlagotheralgos=""
fixedlagcsalgos=""
fixedsinglemultisiedgeoptmyalgos=""
fixedmultimultisiedgeoptmyalgos=""
fixedmultimultisimatrixoptmyalgos=""
sampleintervals=""
missingcompletemode=""
...
if __name__ == "__main__":
    ...
    fixedmacropartialoptmyalgos=cPickle.load(infile)
    fixedmacropartialcovermyalgos=cPickle.load(infile)
    fixedmacropartialotheralgos=cPickle.load(infile)
    fixedmacropartialcsalgos=cPickle.load(infile)
    fixedlagoptmyalgos=cPickle.load(infile)
    fixedlagcovermyalgos=cPickle.load(infile)
    fixedlagotheralgos=cPickle.load(infile)
    fixedlagcsalgos=cPickle.load(infile)
    fixedsinglemultisiedgeoptmyalgos=cPickle.load(infile)
    fixedmultimultisiedgeoptmyalgos=cPickle.load(infile)
    fixedmultimultisimatrixoptmyalgos=cPickle.load(infile)
    ...
    probtype=cPickle.load(infile)
    infile.close()

    if infertype not in ["edge"]:
       assert graphevolution not in ["dynamic"]
    ...
    for tracefilename in myutil.listfiles(newtracefolder):
        if tracefilename.find("info.unique")!=-1:
           continue 
        ...
        #info file also for "singlemulti-si", "multimulti-si","multimulti-si-corr"
        if (infertype in ["edge"] and spreadmodel in ["singlemulti-si", "multimulti-si","multimulti-si-corr"]) and (infertype in ["spreadprobunknown","difprobpartial","lag","macropartial"]):
           uniquefilename="info.unique"
           uniquefilepath="{0}/{1}".format(newtracefolder,uniquefilename)
           infile=gzip.open(uniquefilepath,"rb")
           uniquecount=cPickle.load(infile) #hash keyed by nodenumber and params.value will be list since there might be more than one spread from same node with same parameter
           infile.close()
        ...
        if spreadmodel not in ["model1sum","model2sum","singlemulti-si","multimulti-si","multimulti-si-corr"]:
           startnodes=set()
           for node in mytraces.keys():
               curtime=mytraces[node]["infect"]
               if type(curtime)==list:
                  if 0 in curtime:
                     startnodes.add(node) 
               else:    
                  if curtime==0:
