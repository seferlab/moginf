...
def all_shortest_paths2(G,a,b): 
    ...
    if a not in pred:  # b is not reachable from a 
        return [] 
    pth = [[a,0]] 
    ...
    while ind >= 0: 
        n,i = pth[ind] 
        if n == b: 
            ret.append([x[0] for x in pth[:ind+1]]) 
        if len(pred[n]) > i: 
            ind += 1 
            if ind == pthlength: 
...
def functioncalculator2(depth,Ontology):
    ...
    leng=1
    temp=[]
    mylist=list(Ontology[root].keys())

    while(leng<depth):
      for node in mylist:
        for node2 in list(Ontology[node].keys()):
...
def mipscomplexontoreader(schemefilename,root="root"):
    ...
    for myid in list(id2names.keys()):
...
def molecularontoreader(filename,connected,booleannames=False):
    ...
    if connected:
       ...
       for onto in list(onto2name.keys()):
...
def biogridreadercombinednodename(filename,aliasseperator): 
    ...
    for line in file:
        ...
        if flag: 
            ...
            if len(splitted)!=0 and (splitted[6] in biogrid_physical):
               ...
               if ( first not in aliashash ):
                  aliashash[first]=set()
               if ( second not in aliashash ):
    ...
    #clean aliashash
    for node in list(aliashash.keys()):
...
def biogridreadergeneric(filename):
    ...
    for pro in list(aliashash.keys()):
        if pro not in phy.nodes():
           del aliashash[pro]
    #testing  
    for node in phy.nodes():
        assert node in aliashash
...
def readgraphmyline(linefilename,compressed,lineseperator="?"):
    ...
    for line in file:
        ...
        if cn==0:
           ...
           continue
        if line.startswith("#"):
           for index in range(0,len(list(index2node.keys()))):
        ...
        if not edgeflag:
           ...
           name=line[look:]
           assert index not in index2node
...
def write2files(combinedphy,filename,aliasout=True):
    ...
    if aliasout:
        ...
        file=open(outfile,'w')
        for key in list(aliashash.keys()):
...
def listfiles(dir):
     ...
     for d in dirList:
         if not os.path.isdir(d) and (not d.startswith(".DS_Store")):
            ret.append(d)
     return ret
...
def preprocessanno(pro2anno,allannotations,atleastnumber):
    if allannotations=="-1":
       allannotations=[item for sublist in list(pro2anno.values()) for item in sublist]

    deleteanno=[]
    for anno in allannotations:
        seencount=0 
        for pro in list(pro2anno.keys()):
...
def annofilewriter(filename,pro2function,G,indices="-1"):
    if indices=="-1":
       ...
       for pro in list(pro2function.keys()):
           if pro in G.nodes():
              outstr=str(pro)
...
def readcomplexannotations(annofilename):
    ...
    for line in file:
        ...
        assert nodename not in pro2function
...
def readGOannotations(filename,specificanno,annotations,aliasseperator,Ontology="-1"):
    ...
    pro2function={}
    for node in list(temppro2function.keys()):
        nodename=str(node)
...
def estimate2file(estimated,filename):
    file=open(filename,"w")
    for pro in list(estimated.keys()):
        temp=str(pro)
